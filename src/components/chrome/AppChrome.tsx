...
export function AppChrome({ page, children }: Props) {
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden bg-white">
      <header className="relative z-10 shrink-0 border-b border-ws-line/50 bg-white/95 px-[var(--content-pad-x)] pb-2 pt-[calc(var(--safe-top)+0.65rem)] backdrop-blur">
        <div className="mx-auto flex w-full max-w-[var(--app-max)] items-start justify-between gap-4">
          <div className="page-curve min-w-0 pl-8 pt-1">
            ...
            <h1 className="font-display text-[clamp(1.35rem,1.1rem+1.6vw,2.35rem)] font-light tracking-wide text-ws-blue">
              {title}
            </h1>
          </div>
          <WellspectMark />
      ...
      <main
        id="innehall"
        className="relative mx-auto w-full max-w-[var(--app-max)] flex-1 overflow-y-auto px-[var(--content-pad-x)] py-4 sm:py-5"
      >
        {children}
      </main>

      <footer className="shrink-0 border-t-2 border-ws-blue bg-white px-[var(--content-pad-x)] pb-[calc(var(--safe-bottom)+0.55rem)] pt-2">
        <nav
          aria-label="Appnavigering"
          className="mx-auto flex w-full max-w-[var(--app-max)] items-center justify-between gap-2"
        >
...
function ChromeLink({
  ...
  return (
    <Link
      ...
      aria-label={label}
      title={label}
      className="tap-target inline-flex items-center justify-center rounded-full text-ws-blue transition hover:bg-ws-blue-soft"
    >
      {children}
    </Link>
  );
