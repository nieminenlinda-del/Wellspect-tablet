...
export function ContentMapView() {
  ...
  return (
    <ol className="mx-auto max-w-[var(--read-max)] space-y-2">
      {entries.map((entry) => (
        <li key={entry.path}>
          <Link
            href={entry.path}
            className="flex min-h-[var(--tap-min)] items-baseline justify-between gap-3 rounded-xl px-2 py-2 hover:bg-ws-blue-soft"
          >
